...
export function AnimatedTitle() {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      if (i >= TEXT.length) {
        ...
        doneTimer = window.setTimeout(() => setDone(true), 300);
      }
  ...
  return (
    <h1 className="relative mx-auto max-w-[14ch] text-center text-5xl leading-[1.1] font-black sm:text-7xl">
      ...
      <span aria-hidden className={`ring-title block${done ? " animate-title-ring-pulse" : ""}`}>
        {typed}
        {!done && <span className="animate-caret ml-0.5 inline-block ring-title">|</span>}
      </span>
    </h1>
  );
}
